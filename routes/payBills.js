...
module.exports = [
 ...
 function(session) {
  if (conf.get(session.message.user.id)) {
   builder.Prompts.text(session, 'Please enter biller name');
  } else {
 ...
 function(session, results, next) {
  ...
  session.send('Do you want to pay the bill for \'%s\'', results.response);
  next();
 },

 // Amount
 function(session) {
  builder.Prompts.number(session, 'Enter bill amount?');
 },
 function(session, results, next) {
 ...
 function(session) {
  ...
  session.send('Do you want to pay the amount\'%s\' to \'%s\'', amount, destination);
  builder.Prompts.confirm(session, 'Do you want to continue?');

 },
 function(session, results) {
  if (results.response) {
   session.send('Bill amount \'%s\' paid to \'%s\' successfully', amount, destination);
  } else {
   // Always say goodbye
